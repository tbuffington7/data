CREATE MATERIALIZED VIEW consolidated_hr_parcels_swg AS 
 WITH p0 AS (
         SELECT p.state_code,
            p.cnty_code,
            p.risk_class,
 ...
 SELECT min(h.parcel_id) AS p_id,
    ...
    p1.geom
   FROM p1
     LEFT JOIN hr_parcels_swg h ON p1.geom && h.wkb_geometry AND p1.state_code::text = h.state_code::text AND p1.risk_class = h.risk_class AND p1.res_other = h.res_other
  GROUP BY p1.state_code, p1.cnty_code, p1.risk_class, p1.res_other, p1.path, p1.geom
WITH DATA;

ALTER TABLE consolidated_hr_parcels_swg
  OWNER TO sgilbert;
COMMENT ON MATERIALIZED VIEW consolidated_hr_parcels_swg
...
-- Index: ndx_consolidated_geom

-- DROP INDEX ndx_consolidated_geom;

CREATE INDEX ndx_consolidated_geom
  ON consolidated_hr_parcels_swg
  USING gist
  (geom);
